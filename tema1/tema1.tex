   ...
   \begin{problema}
     ...
     \item Demostrar que existen exactamente dos soluciones,
       ...
       \begin{solucion}
         ...
         \begin{enumerate}
         \item $\alpha_1\in [-1,0]$, pues $f(-1)=e^{-5}+1>0$ y $f(0)=e^{-3}-1<0$.
         \item $\alpha_2\in [2,3]$, pues $f(2)=e-5<0$ y $f(3)=e^{3}-7 >
           0$.
         \end{enumerate}
       \end{solucion}
     ...
     \item Localizar un intervalo $[a,b]$ que contenga a $\alpha_1$,
       ...
       \begin{solucion}
         Tomamos $[a,b]=[-1,0]$. Entonces:
         \begin{enumerate}
         \item $g([a,b])\subset [a,b]$. En efecto, $g$ es monótona (creciente) porque
           $$
           g'(x)=2e^{2x-3}>0,
           $$
           y además $g(-1)=e^{-5}/2 - 1/2 \in[-1,0]$ y
           $g(0)=e^{-3}/2-1/2  \in [-1,0]$.
         \item $g$ es contractiva con constante $\lambda<1$. Para ello es
           suficiente ver que $\lambda=\max_{x\in[a,b]} |g'(x)|<
           1$. Como $$g''(x)=4e^{2x-3}>0,$$ entonces $g'$ es creciente,
           luego alcanza su máximo en $x=0$ y $g'(0)=2e^{-3}
           \approx 0.099<1$. Podemos tomar $\lambda=0.1$ como cota de
           la derivada, y por tanto como constante de contractividad en
           $[a,b]$.
         \end{enumerate}
       \end{solucion}
     ...
     \item Sea $x_0\in [a,b]$ y sea $x_k$  la sucesión de
       ...
       \begin{solucion}
         (a)
         Por definición, $x_k=g(x_{k-1})$. Además, como $\alpha_1$ es un
         punto fijo de $g$, $g(\alpha_1)=\alpha_1$. Aplicando lo
         anterior y la contractividad de $g$ (repetidamente):
         \begin{align*}
           |x_k -\alpha_1| &= |g(x_{k-1})-g(\alpha_1)| \le
                             \lambda|x_{k-1}-\alpha_1| \\
                           &= \lambda |g(x_{k-2})-g(\alpha_1)| \le
                             \lambda^2 |x_{k-2}-\alpha_1| \le \cdots \le \lambda^k
                             |x_0 - \alpha_1|.
         \end{align*}
         (b)
         Por otra parte, usando que $x_0, \alpha_1\in [a,b]$,
         \begin{equation*}
           |x_k -\alpha_1| < \lambda^k |x_0 - \alpha_1| \le \lambda^k (b-a),
         \end{equation*}
         por tanto para que $|x_k -\alpha_1| < \varepsilon = 10^{-5}$, es suficiente que
         \begin{equation*}
           \lambda^k < \frac{\varepsilon}{(b-a)}.
         \end{equation*}
         Para ello, basta tomar $k$ como el primer entero mayor que $k_0$, con
         \begin{equation*}
           k_0\in\Rset \text{tal que } \lambda^{k_0} = \frac{\varepsilon}{(b-a)},
         \end{equation*}
         es decir, para $\varepsilon=10^{-5}$, $[a,b]=[-1,0]$ y $\lambda=0.1$,
         \begin{equation*}
     ...
     \item Demostrar que la aproximación mediante el método de Newton
       ...
       \begin{solucion}
         ...
         \begin{align*}
           ...
           f''(x)&=4e^{2x-3}. \\
         \end{align*}
         Para $x_0=3$, se satisfacen las hipótesis de la Regla de Fourier en $[2,3]$:
         \begin{enumerate}
         \item $f(2)=e-5<0$, $f(3)=e^3-7>0$.
         \item $f'$ es creciente, por lo tanto $f'(x)>f(2)=2e-2>0$ en $[2,3]$.
         \item $f''(x)>0$ en $[2,3]$.
         \item Como $f(3)>0$ y $f''(3)>0$, se verifica $f(3)f''(3)>0$.
         \end{enumerate}
         En consecuencia, el método de Newton es
         convergente.

         (b) Realicemos las dos primeras iteraciones:
         \begin{enumerate}
         \item $x_1 = x_0 - f(x_0)/f'(x_0) = 3 - (e^3-7)/(4e^3-2)
           \approx 2.65718708947.$
