...
\begin{example}
  ...
  \item Por último, el teorema de Bolzano nos permitirá determinar dos
    ...
    \begin{itemize}
    \item $[a_1,b_1]=[-\pi/2, 0]$, pues $f(-\pi/2)=-1-4\pi^2+1<0$ y
      $f(0)=1>0$,
    \item $[a_2,b_2]=[0, \pi/2]$, pues $f(0)=1>0$ y $f(\pi/2)= 1-4\pi^2+1<0$
    \end{itemize}
    ...
    \item $[a_1,b_1]=[-\pi/4, 0]$. Seguimos estando en las hipótesis
      ...
      -\sqrt{2}/2-4\pi^2+1=-2.174...<0$ y $f(0)=1>0$,
    \end{itemize}
  \end{enumerate}
...
\begin{proof}
  ...
  hipótesis~\ref{item:Newton:H4} implica:
  \begin{equation}
    \frac{f(b)}{f'(b)} < b-a.
