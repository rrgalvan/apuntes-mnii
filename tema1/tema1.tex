...
\begin{remark}[Separación de ceros]
  ...
  % ejemplos.
  Suponiendo la continuidad y derivabilidad de $f$ necesaria (para
...
\begin{remark}[Orden de~(\ref{eq:MAS})]~
  \begin{enumerate}
  \item En las hipótesis del Teorema~\ref{thm:punto-fijo-Banach} los
  ...
  \item Para hipótesis más restrictivas sobre $g$ se puede llegar a
    \resaltar{orden mayor que uno}. En concreto, se puede demostrar
    que si~(\ref{eq:MAS}) converge a un punto fijo, $\cero$, y si $g\in
    C^p([a,b])$ siendo $p \ge 1$ un entero, con
    \begin{extension}
...
\begin{example}
  ...
  \item Usando de nuevo el
    ...
    (separados por el cero de
  \item Por último, el teorema de Bolzano nos permitirá determinar dos
    intervalos $[a_1,b_1]$ y $[a_2,b_2]$ que contengan estos dos
    ceros. Podemos proceder mediante prueba y error, o bien utilizar
    un programa de ordenador para representar la gráfica como
    ayuda. Así, una elección válida es, por ejemplo,
    \begin{itemize}
  ...
  estos intervalos, el problema en términos de un esquema de punto en
  las hipótesis del teorema~\ref{thm:punto-fijo-Banach}. La
  ...
  $$
  g_1'(x)=\frac{-\cos(x)}{\sqrt{\sen(x)+1}},
  $$
  que está bien definida para todo $x\in[-1, 0]$ (ya que  $-\pi/2 <-1
  \le x \le 0$ y entonces  $\sen(x)+1>0$). Es más:
  \begin{enumerate}
  \item $g_1([a_1,b_1])\subset [a_1,b_1]$: En efecto, $g_1$ es
    decreciente en $[-1,0]$ (pues si $x\in [-1, 0]$, $\cos(x)\ge 0$ y
    por tanto $g_1'(x)\le 0$). Por tanto, como $g_1(-1)=-0.398$ y
    $g_1(0)=-1.0$, se tiene que $g([-1,0])\subset [-1.0,-0.398]
    \subset [-1,0]$.
  \item Veamos que $g_1$ es contractiva en $[a_1,b_1]$. Como $\sen(x)$
    es creciente en $[-\pi/2,0]$ (y en particular en $[-1,0]$), para
    todo $x\in[-1,0]$ se tiene que
    $\sqrt{\sen(x)+1}\ge\sqrt{\sen(-1)+1}>1$, así
    $$
    \left|g'(x)\right| = \frac{|\cos{x}|}{\sqrt{\sen(x)+1}} \le
    \frac{1}{\sqrt{\sen(-1)+1}} =: L < 1,
    $$
    es decir $\sup_{x\in(-1,0)} |g'(x)| = \lambda \le L < 1$. Gracias a la
    proposición~\ref{pro:1}, concluimos que $g_1$ es contractiva.
  \end{enumerate}
...
\begin{example}
  \label{sec:metodos-de-punto}
  Demostrar que el método~(\ref{eq:MAS}) converge para $g(x)=$
\end{example}
