...
\begin{proof}
  ...
  % el error de interpolación, $f(x)-p_n(x)$, se puede escribir de
  % la forma $\lambda\, w_S(x)$, esto es
  \begin{equation*}
  ...
  \begin{equation}
    ...
    \label{eq:tema2.2}
  \end{equation}
  O sea, tenemos que hallar $\lambda$ % para que se
  % verifique~(\ref{eq:tema2:1}), es decir
  ...
  Evidentemente, % para que $x$ verifique $\phi_\lambda(x)=0$,
  basta tomar
  $$
