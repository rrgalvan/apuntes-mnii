...
\begin{enumerate}
\item La expresión del error~(\ref{eq:error.fc.compuesta-NC}) nos garantiza la
  convergencia de $I_N(f)$ hacia $\int_a^b f(x)\, dx$ cuando $h\to 0$
  (o, equivalentemente, cuando $N=(b-a)/h \to\infty$).
\item Más aún,~(\ref{eq:error.fc.compuesta-NC}) nos indica cuál es exactamente la
...
\begin{example}[Fórmula compuesta del punto medio]
  ...
  \begin{equation*}
    \int_{j-1}^j f(x)\, dx \approx I_1(f)=
    (x_j-x_{j-1}) \, f\big(\frac{x_j+x_{j-1}} 2 \big) = 
    2hf_j,
  \end{equation*}
  donde $h=(x_j-x_{j+1})(n+2)=(x_j-x_{j+1})/2$ y definimos $f_j=f
  \big(\frac{x_j+x_{j-1}} 2 \big)=f(x_{j+1}+h)$.
\end{example}
