...
\begin{proof}~
  ...
  \begin{equation}
    ...
    f''(\xi)=\frac 1N \sum_{i=1}^Nf^{k)}(\xi_i).
  \end{equation}
  Como $f\in C^k([a,b])$, $f^{k)}$ es continua en $[a,b]$ y por lo tanto
  alcanza todos los valores entre su máximo y su mínimo. Sean
  $m_2=\min_{[a,b]} f^{k)}(x)$ y $M_2=\max_{[a,b]} f^{k)}(x)$. Entonces
  $$
  m_2 \le \frac 1N \sum_{i=1}^Nf^{k)}(\xi_i) \le M_2,
  $$
...
\begin{example}[Fórmula compuesta del punto medio]
  ...
  \end{equation*}
  Para utilizarla dentro de una f.c. compuesta, denotamos
  $$
  h=\frac{\ox_{i+1}-\ox_{i}}{n+2}=\frac{\ox_{i+1}-\ox_{i}}2 = \frac{b-a}{2N},
  $$
  ...
  \end{equation*}
  Procedemos como en la demostración del teorema anterior, tenemos que
  existe $\xi\in(a,b)$ tal que
  $$\sum_{i=0}^{N-1}f''(\xi_i)=Nf''(\xi)=\frac{b-a}{2h},$$
  y así llegamos a la expresión definitiva:
  \begin{equation*}
    \int_a^b f(x)\,dx 
    = 2h \sum_{i=0}^{N-1} f_{2i+1} + \frac{h^3(b-a)}{6} f''(\xi).
  \end{equation*}  
\end{example}
