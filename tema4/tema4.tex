...
caso general, donde $f:[\ta,\tb]\times\Rset^n\to\Rset^n$ e
...
\label{pro:propiedades-pvi}
  ...
  \item Sean $\sol_1(\tt)$ y $\sol_2(\tt)$ las soluciones de dos
    problemas del tipo~\eqref{eq:pvi} para la misma ecuación
    diferencial $y'=f(\tt,y)$ pero con distintas condiciones
    iniciales, es decir:
    $$\sol_1(\ta)<\sol_2(\ta)$$ 
  ...
  \item Sean $\sol_1(\tt)$ y $\sol_2(\tt)$ las soluciones de dos
    problemas del tipo~\eqref{eq:pvi} con el mismo dato inicial pero
    con ecuaciones diferenciales diferentes, $y'=f_1(\tt,y)$
    $y'=f_2(\tt,y)$. Si
    $$
...
\begin{center}
  \begin{graficaTikz}[width=23em, height=17em]
    \begin{axis}[ \axisXYmiddle, xtick=\empty, ytick=\empty, legend
      ...
      \addplot[domain=1.5:3, red, thick] {36+32*(x-2)};
      % Plot a label at curve root
      \node[coordinate, medium dot, pin=100:{\scriptsize $\sol(\tt_n)$}] at
      (axis cs:2,36) {}; 
